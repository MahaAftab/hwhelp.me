...
$sql = "SELECT * FROM vedio WHERE courseid = '$course_id'";
$result = mysqli_query($con, $sql);

// Display quizzes as hyperlinks with their quiz title and course name
while ($row = mysqli_fetch_assoc($result)) {
    $vedioid = $row['vedioid'];
    $vediotittle = $row['vediotittle'];
    $vediodescription = $row['vediodescription'];
   
    $url = "vedioplay.php?vedioid=" .$vedioid;
?>
     <div class="row chapter" style="margin: 5px !important;">
        <div class="col-sm-2">
            <?php echo '<h6> <b> ' . $vediotittle . ' </b></h6>'; ?>
        </div>
        <div class="col-sm-7">
            <?php echo '<h6> <b> ' . $vediodescription . ' </b></h6>'; ?>
        </div>
        <div class="col-sm-3">

        <?php echo '<td><a href="ved.php?vedio=' . $row["vedio"] . '"> <h6> <b> View and download </b></h6></a> <td>'; ?>
